...
function Bar({ value, mastered }: { value: number; mastered: boolean }) {
  return (
    <div className="h-2 w-full overflow-hidden rounded-full bg-zinc-800">
      <div
        className={`h-full rounded-full transition-all ${mastered ? "bg-emerald-400" : "bg-white"}`}
    ...
    <div className="space-y-4">
      <div className={`${CARD} p-5`}>
        <div className="mb-1 flex items-baseline justify-between">
          ...
          <span className="text-3xl font-bold">{Math.round(p.percent)}%</span>
        </div>
            ...
            <div className="mb-1 flex items-center justify-between gap-2">
              ...
              <span className="shrink-0 text-sm">
                {c.mastered ? (
                  ...
                  <span className="text-zinc-500">{Math.round(c.masteryScore * 100)}%</span>
                )}
              </span>
            </div>
          ...
          <ul className="space-y-1 text-sm">
            {data.recentAttempts.slice(0, 10).map((a) => (
              <li key={a.id} className="flex justify-between gap-2 text-zinc-300">
                ...
                <span className={a.score >= 0.8 ? "font-semibold text-emerald-400" : "font-semibold text-amber-400"}>
